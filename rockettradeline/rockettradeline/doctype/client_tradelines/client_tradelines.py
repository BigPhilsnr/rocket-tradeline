...
class ClientTradelines(Document):
    ...
    def validate_discount(self):
        ...
        if self.discount_type:
            if not self.discount_value:
                frappe.throw("Discount value is required when discount type is selected")
            ...
            if self.discount_type == "Percentage":
                ...
                if flt(self.discount_value) > subtotal:
                    frappe.throw(f"Discount amount cannot exceed subtotal of {subtotal}")
    
    def calculate_amounts(self):
        """Calculate subtotal, discount amount, and total amount"""
        if not self.quantity or not self.unit_price:
            return
        
        # Calculate subtotal
        self.subtotal = flt(self.quantity) * flt(self.unit_price)
    ...
    def handle_status_change(self):
        ...
        elif old_status != "Pending AU" and self.status == "Pending AU":
            # Send AU assignment notification email to cardholder (only if not external import)
            if not self.is_external:
                try:
                    self.send_au_assignment_email()
                    print(f"AU assignment notification processed for client tradeline {self.name}")
                ...
                print(f"Skipped AU assignment notification for external import: {self.name}")
        
        elif self.status == "Refund Requested":
            # Send refund request notification email to admin (only if not external import)
            if not self.is_external:
                try:
                    self.send_refund_request_notification_email()
                    print(f"Refund request notification processed for client tradeline {self.name}")
                except Exception as e:
                    frappe.log_error(
                ...
                print(f"Skipped refund request notification for external import: {self.name}")
        
        elif self.status == "Active":
            # Check if status changed from "Refund Requested" to "Active"
            if old_status == "Refund Requested":
                # Send Non-Posting Assistance Resolution email
                if not self.is_external:
                    try:
                        self.send_non_posting_resolution_email()
                        print(f"Non-Posting Assistance Resolution email sent for client tradeline {self.name}")
                    except Exception as e:
                        frappe.log_error(
                            f"Failed to send Non-Posting Assistance Resolution email for client tradeline {self.name}: {str(e)}", 
                            "Non-Posting Resolution Email Error"
                        )
                else:
                    print(f"Skipped Non-Posting Assistance Resolution email for external import: {self.name}")
            else:
                # Send active status notification email to customer (only if not external import)
                if not self.is_external:
                    try:
                    ...
                    print(f"Skipped active status notification for external import: {self.name}")
        
        elif self.status == "Removed":
            # Send removal confirmation email to customer (only if not external import)
            if not self.is_external:
                try:
                ...
                print(f"Skipped removal confirmation notification for external import: {self.name}")
    ...
    def recalculate_tradeline_remaining_spots(self):
        ...
        try:
            ...
            if new_remaining_spots < 0:
                frappe.throw("Error: Remaining spots cannot be negative total purchased "+ str(new_remaining_spots))
    ...
    def send_au_assignment_email(self):
        ...
        try:
            ...
            user_dob = frappe.db.get_value("User", au_customer_doc.email_id, "birth_date")
            if user_dob:
                from frappe.utils import formatdate
                au_dob = formatdate(user_dob, "MM/dd")
    ...
    def send_au_assignment_reminder_email(self):
        """
        Send AU assignment reminder email to the cardholder
        This method gathers all required data and sends the email using the AU Assignment Reminder template
        """
        try:
            # Get the tradeline details
            tradeline_doc = frappe.get_doc("Tradeline", self.tradeline)
            ...
            # Send the email using the reminder template
            from rockettradeline.utils.email_templates import send_email_from_template
            
            result = send_email_from_template(
                template_name='AU Assignment Reminder',
                recipients=[cardholder_doc.email_id],
                context=context
            )
            ...
            if result.get('success'):
                ...
                self.add_comment(
                    "Info", 
                    f"AU assignment reminder email sent to cardholder {cardholder_doc.email_id} "
                    f"for AU {au_first_name} {au_last_initial}."
                )
                frappe.logger().info(
                    f"AU assignment reminder email sent successfully to {cardholder_doc.email_id} "
                    f"for client tradeline {self.name}"
                )
                return True
            else:
                frappe.log_error(
                    f"Failed to send AU assignment reminder email to {cardholder_doc.email_id} "
                    f"for client tradeline {self.name}: {result.get('error')}", 
                    "AU Assignment Reminder Email Error"
                )
                return False
                
        except Exception as e:
            frappe.log_error(
                f"Error sending AU assignment reminder email for client tradeline {self.name}: {str(e)}", 
                "AU Assignment Reminder Email Error"
            )
            return False
    ...
    def send_active_status_notification_email(self):
        ...
        try:
            ...
            if tradeline_doc.closing_date:
                from frappe.utils import formatdate
                day = int(tradeline_doc.closing_date)
                if day == 1:
                    closing_day = "1st"
                elif day == 2:
                    closing_day = "2nd"
            ...
            # Get mailing address from tradeline
            mailing_address = ""
            if tradeline_doc.mailing_address:
                address_doc = frappe.get_doc("Address", tradeline_doc.mailing_address)
                address_parts = []
            ...
            # Send the email using the template
            from rockettradeline.utils.email_templates import send_email_from_template
            recipient = customer_doc.account_manager or customer_doc.email_id
    ...
    def send_non_posting_resolution_email(self):
        """
        Send Non-Posting Assistance Resolution email to customer
        This method is triggered when status changes from "Refund Requested" to "Active"
        """
        try:
            # Get the tradeline details
            tradeline_doc = frappe.get_doc("Tradeline", self.tradeline)
            ...
            # Get current date/time for resolved date
            ...
            context = {
                ...
                'resolution_type': 'Status Change from Refund Requested to Active',
                'resolution_notes': f"The tradeline has been successfully reactivated and is now showing as Active. Previous refund request has been resolved.",
                'customer_email': customer_doc.email_id or "N/A",
                'tradeline_id': self.tradeline,
                'tradeline_name': self.tradeline_name,
                'bank_name': bank_doc.bank_name if bank_doc else tradeline_doc.bank,
                ...
                'tradeline_age': age_display,
                'final_status': 'Active',
                'final_notes': 'Your tradeline is now active and posting to your credit report. You can monitor its status in your account dashboard.'
            }
            ...
            result = send_email_from_template(
                template_name='Non-Posting Assistance Resolution',
                recipients=[customer_doc.email_id],
                context=context
            )
            ...
            if result.get('success'):
                ...
                self.add_comment(
                    "Info", 
                    f"Non-Posting Assistance Resolution email sent to customer {customer_doc.customer_name} "
                    f"({customer_doc.email_id}) after status change from Refund Requested to Active."
                )
                print(
                    f"Non-Posting Assistance Resolution email sent successfully to {customer_doc.email_id} "
                    f"for client tradeline {self.name}"
                )
                return True
            else:
                frappe.log_error(
                    f"Failed to send Non-Posting Assistance Resolution email to {customer_doc.email_id} "
                    f"for client tradeline {self.name}: {result.get('error')}", 
                    "Non-Posting Resolution Email Error"
                )
                return False
            ...
            frappe.log_error(
                f"Error sending Non-Posting Assistance Resolution email for client tradeline {self.name}: {str(e)}", 
                "Non-Posting Resolution Email Error"
            )
            return False
...
def test_send_verification_email_simple():
    doc = frappe.get_doc("Client Tradelines","CT-00000084" )
    doc.send_active_status_notification_email()