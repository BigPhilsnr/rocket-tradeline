...
def send_email_template(template_name, recipients, parameters=None, **kwargs):
	"""
	Wrapper function for sending emails using Email Template Custom
	
	Args:
		template_name (str): Name of the email template
		recipients (list): List of email addresses  
		parameters (dict): Parameters/context for template rendering
		**kwargs: Additional arguments for frappe.sendmail
	...
	try:
		...
		success = template.send_email(recipients, parameters, **kwargs)
		return success
		
	except Exception as e:
		frappe.log_error(f"send_email_template failed for '{template_name}': {str(e)}", "Email Template Wrapper Error")
		return False